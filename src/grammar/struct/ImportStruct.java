...
public class ImportStruct extends Struct {
	
	private static final int EXPECT_IMPORT = 1,
			EXPECT_NAME = 2,
			EXPECT_SEMICOLON = 3,
			EXPECT_SEMICOLON_DOT = 4;
	private int status = EXPECT_IMPORT;
	private Node head;
	...
	public boolean accept(Unit unit) {
		...
		case EXPECT_NAME:
			if (Unit.EWORD == type && 
				...
				this.status = EXPECT_SEMICOLON_DOT;
				Node node = new Node();
				node.content = word;
				if (head == null) {
					head = node;
